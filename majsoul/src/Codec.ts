...
export class Codec {
	...
	public static decode(root: Root, wrapper: Type, data: Buffer): any {
		...
		const type = root.lookupType((message as any)["name"]);
	...
	public decodeMessage(message: Buffer, methodName?: string): {
		...
		const method = this.lookupMethod(methodName || (unwrappedMessage as any)["name"]);
		return {
			type,
			index,
			data: this.protobufRoot.lookupType(type === MessageType.Response
				? method.requestType
				: method.requestType).decode((unwrappedMessage as any)["data"])
		};
	}
